 ...
 *
 * Run with:
 *   npm run decompile                 # the two SSM simulators → NAAP/decompiled/
 *   npm run decompile -- --all        # all SSM-relevant movies
...
const SSM_TARGETS = [
  ...
  "ptolemaic/ptolemaic023-C.swf", // Ptolemaic System Simulator
  "configurationsSimulator/configurationsSimulator044-C.swf", // Planetary Configurations Simulator
  // Supporting concept demos composed by / behind the two simulators.
  "simpleSolarSystem/simpleSolarSystem001.swf",
  "elongationComponent/elongationComponent006.swf",
  "solarSystemProperties/solarSystemProperties003-B.swf",
].map((p) => join(flashRoot, p));

/** The two simulators being ported, decompiled by default when no targets are given. */
const DEFAULT_TARGETS = SSM_TARGETS.slice(0, 2);
...
Usage:
  npm run decompile                  Decompile the two SSM simulators → NAAP/decompiled/
  npm run decompile -- --all         Decompile all SSM-relevant movies
  npm run decompile -- <path>…       Decompile specific .swf / .fla / folder(s)

Flags:
  --all              Decompile the curated set of SSM-relevant SWFs

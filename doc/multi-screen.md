...
main.ts
  └─ SolarSystemModelsScreen            (Screen<SolarSystemModelsModel, SolarSystemModelsScreenView>)
       ├─ SolarSystemModelsModel         owns all state
       └─ SolarSystemModelsScreenView    owns all visuals
```
...
existing `SolarSystemModelsScreen.ts`.
...
const icon = new ScreenIcon(
  new Rectangle(0, 0, 548, 373, { fill: SolarSystemModelsColors.accentColorProperty }),
